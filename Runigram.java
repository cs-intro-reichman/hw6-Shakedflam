...
public class Runigram {
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i=0; i<height; i++){
			for (int j=0; j<width;j++){
				int originalHeight= (int) (((i/(double)height))*sourceHeight);
				int originalWidth= (int) (((j/(double)width)) * sourceWidth);
				scaledImage[i][j]= image[originalHeight][originalWidth];
			}
			System.out.println();
		} 
		return scaledImage;
	...
	public static Color[][] blend(Color[][] image1, Color[][] image2, double alpha) {
		...
		Color [][] blendedImage= new Color[rows][col];
		for (int i=0; i<rows; i++){
			for (int j=0; j<col; j++){
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		if (sourceHeight!=target.length|| sourceWidth!=target[0].length){
			target= scaled(target, sourceWidth, sourceHeight);  
		}
		double alpha = 1.0;
		for (int i=0; i<=n; i++){
			Color[][] morphColors= blend(source, target, alpha);
			Runigram.display(morphColors);
			alpha-= 1/(double)n;
			StdDraw.pause(500);
		}
